...
// Mapeamento de Cores para o Discord (Decimal)
const DiscordColors = {
    ONLINE: 5763719,   // Verde (0x57F287)
    RECEBIDO: 3447003, // Azul (0x3498DB)
    ENVIADO: 16776960, // Amarelo (0xFFFF00)
    ERRO: 15548997,    // Vermelho (0xED4245)
    INFO: 9807270      // Cinza (0x95A5A6)
};

async function sendToDiscord(tipo, titulo, detalhe) {
    if (!WEBHOOK_URL || WEBHOOK_URL === "https://discordapp.com/api/webhooks/1461009453410291826/deimejV9KMK2QuAcYn33OlS_i_yZy0RUZfJifI7MBtWh6-5y349NLNkX3S3MQikSTTOg") return;
    ...
    const payload = {
        embeds: [{
            ...
            description: detalhe ? `\`\`\`${detalhe}\`\`\`` : undefined, // Coloca o detalhe em bloco de código
            color: cor,
    ...
    try {
        ...
        console.log(`${C.red}[ERRO DISCORD] Falha ao enviar log: ${err.message}${C.reset}`);
    }
}
...
wppconnect
  ...
  .catch((error) => logSystem('ERRO', 'Falha ao iniciar', error.message));

function start(client) {
  logSystem('ONLINE', 'Sistema Iniciado', `Sessão: ${NOME_EMPRESA}`);
  ...
  client.onMessage(async (message) => {
    ...
    // Só loga no Discord mensagens novas (evita spam de status)
    ...
    if (userStage === 'INICIO') {
        ...
        try {
            ...
            logSystem('ERRO', 'Falha Lista (iPhone?)', userId);
    ...
    // --- ESTÁGIO 3: FINALIZADORES ---
        ...
        if (nomeEscolhido) {
            await client.sendText(userId, `*${nomeEscolhido.toUpperCase()}*\n__________________________\n\n👋 Olá! Vi que quer falar comigo. Já recebi sua notificação!`);
